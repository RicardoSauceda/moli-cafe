...
@section('title', 'Crear Promoción | MoLi Café')
...
<div class="mb-8">
    <div class="flex items-center gap-3 mb-4">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900">Nueva Promoción</h1>
    </div>
    <p class="text-gray-600">Crea una nueva promoción para mostrar en el landing page</p>
</div>

<form action="{{ route('promotions.store') }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}" 
                       placeholder="Ej: Café Arábica Premium 30% OFF"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" placeholder="Describe la promoción..."
                          class="w-full px-4 py-2 rounded-xl border border-gray-300 focus:border-moli-yellow focus:ring-4 focus:ring-yellow-300/20 outline-none transition">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="promotion_type" name="promotion_type" 
                    ...
                    <option value="percentage" {{ old('promotion_type') === 'percentage' ? 'selected' : '' }}>Porcentaje (%)</option>
                    <option value="fixed_amount" {{ old('promotion_type') === 'fixed_amount' ? 'selected' : '' }}>Monto Fijo ($)</option>
                    <option value="special_price" {{ old('promotion_type') === 'special_price' ? 'selected' : '' }}>Precio Especial ($)</option>
                </select>
            ...
            <div id="discount_percentage_field" style="display: {{ old('promotion_type') === 'percentage' ? 'block' : 'none' }};">
            ...
            <div id="discount_amount_field" style="display: {{ old('promotion_type') === 'fixed_amount' ? 'block' : 'none' }};">
            ...
            <div id="price_field" style="display: {{ old('promotion_type') === 'special_price' ? 'block' : 'none' }};">
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-center gap-3">
                        <input type="radio" id="validity_fixed" name="validity_type" value="fixed" 
                               {{ old('validity_type', 'fixed') === 'fixed' ? 'checked' : '' }}
                    ...
                    <div class="flex items-center gap-3">
                        <input type="radio" id="validity_seasonal" name="validity_type" value="seasonal"
                               {{ old('validity_type') === 'seasonal' ? 'checked' : '' }}
                               onchange="toggleValidityType()">
            ...
            <!-- Fechas -->
            <div id="validity_date_section" style="display: {{ old('validity_type', 'fixed') === 'fixed' ? 'block' : 'none' }};" class="space-y-4">
                <div>
            ...
            <div id="validity_description_section" style="display: {{ old('validity_type') === 'seasonal' ? 'block' : 'none' }};">
                ...
                <input type="text" id="validity_description" name="validity_description" 
                       value="{{ old('validity_description') }}"
                       placeholder="Ej: Todo el verano, Solo fines de semana, Especial Navidad"
                       class="w-full px-4 py-2 rounded-xl border border-gray-300 focus:border-moli-yellow focus:ring-4 focus:ring-yellow-300/20 outline-none transition">
